    ...
    while(true) {
        ...
        switch (command) {
            case 4: { //get coords
                ...
                std::cout << "sent client_ID\n";
                //Sending message
                send(Connection, (char *) &coords_size, sizeof(int), 0);
                std::cout << "sent coords-size: " << coords_size << "\n";
                send(Connection, coords.c_str(), coords_size, 0);
                std::cout << "sent coords-str: " << coords << "\n";
                Sleep(10);
                break;
            }
            case 5: { //get coords cont
                while (true) {
                    ...
                    //Sending client ID
                    send(Connection, (char *) &client_ID, sizeof(int), 0);
                    std::cout << "sent client_ID " << client_ID << " : " << (char*)&client_ID <<  "\n";
                    //Sending message
                    send(Connection, (char *) &coords_size, sizeof(int), 0);
                    std::cout << "sent coords-size: " << coords_size << " : " << (char*)&coords_size << "\n";
                    send(Connection, coords.c_str(), coords_size, 0);
                    std::cout << "sent coords-str" << coords << "\n";
                    Sleep(1000);
                }
